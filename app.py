...
def db():
    ...
    c.execute('CREATE TABLE IF NOT EXISTS customers (slug TEXT PRIMARY KEY, company TEXT NOT NULL, email TEXT, app_url TEXT, started_at TEXT NOT NULL, expires_at TEXT NOT NULL, permanent INTEGER NOT NULL DEFAULT 0, token TEXT NOT NULL UNIQUE)'); c.commit(); return c

def safe_slug(s): return re.sub(r'[^a-z0-9-]','',s.lower().replace(' ','-')) or 'demo'
def now(): return datetime.now(timezone.utc)
def customer(slug):
    c=db(); r=c.execute('SELECT * FROM customers WHERE slug=?',(slug,)).fetchone(); c.close(); return r
...
def create_demo():
    ...
    start=now(); end=start+timedelta(days=7); token=secrets.token_urlsafe(24)
    c.execute('INSERT INTO customers(slug,company,email,app_url,started_at,expires_at,token) VALUES(?,?,?,?,?,?,?)',(slug,company,email,app_url,start.isoformat(),end.isoformat(),token)); c.commit(); c.close()
    url=f'/install/{slug}?token={token}&bedrijf={company}&app={app_url}'
    return jsonify(ok=True,message='7-daagse demo aangemaakt.',install_url=url)

@app.get('/api/admin/customers')
def customers():
    c=db(); rows=c.execute('SELECT * FROM customers ORDER BY started_at DESC').fetchall(); c.close(); out=[]
    for r in rows:
        left=max(0,(datetime.fromisoformat(r['expires_at'])-now()).days+1) if not r['permanent'] else 999
        out.append({'slug':r['slug'],'company':r['company'],'email':r['email'],'active':active(r),'permanent':bool(r['permanent']),'days_left':left,'install_url':f"/install/{r['slug']}?token={r['token']}&bedrijf={r['company']}&app={r['app_url']}"})
    return jsonify(out)
...
def manifest(slug):
    ...
    start=f"/app/{slug}?token={token}"
    return jsonify({'id':f'/app/{slug}','name':f"BouwFlow - {r['company']}",'short_name':'BouwFlow','start_url':start,'scope':'/','display':'standalone','background_color':'#071c38','theme_color':'#071c38','icons':[{'src':'/bouwflow-icon.svg','sizes':'any','type':'image/svg+xml','purpose':'any maskable'}]})
...
def demo():
    ...
    payload={'sender':{'name':BREVO_FROM_NAME,'email':BREVO_FROM_EMAIL},'to':[{'email':DEMO_TO_EMAIL,'name':'BouwFlow'}],'replyTo':{'email':email,'name':naam},'subject':f'Nieuwe gratis demo-aanvraag - {bedrijf}','textContent':text}
