import os
import requests
from flask import Flask, jsonify, request, send_from_directory

app = Flask(__name__, static_folder='.', static_url_path='')

BREVO_API_KEY = os.getenv('BREVO_API_KEY', '').strip()
DEMO_TO_EMAIL = os.getenv('DEMO_TO_EMAIL', 'fokkerrik@gmail.com').strip()
BREVO_FROM_EMAIL = os.getenv('BREVO_FROM_EMAIL', 'fokkerrik@gmail.com').strip()
BREVO_FROM_NAME = os.getenv('BREVO_FROM_NAME', 'BouwFlow').strip()

@app.get('/')
def index():
    return send_from_directory('.', 'index.html')

@app.post('/api/demo')
def demo():
    data = request.get_json(silent=True) or {}
    naam = str(data.get('naam', '')).strip()
    bedrijf = str(data.get('bedrijf', '')).strip()
    email = str(data.get('email', '')).strip()
    telefoon = str(data.get('telefoon', '')).strip()
    soort = str(data.get('soort', '')).strip()
    bericht = str(data.get('bericht', '')).strip()

    if not naam or not bedrijf or not email or not bericht:
        return jsonify(ok=False, message='Vul alle verplichte velden in.'), 400

    if not BREVO_API_KEY:
        return jsonify(ok=False, message='Het contactformulier is nog niet gekoppeld aan de e-mailservice.'), 500

    text = f'''Nieuwe aanvraag via BouwFlow\n\nNaam: {naam}\nBedrijf: {bedrijf}\nE-mail: {email}\nTelefoon: {telefoon or '-'}\nOnderwerp: {soort or '-'}\n\nHuidige werkwijze / vraag:\n{bericht}\n'''

    payload = {
        'sender': {'name': BREVO_FROM_NAME, 'email': BREVO_FROM_EMAIL},
        'to': [{'email': DEMO_TO_EMAIL, 'name': 'BouwFlow'}],
        'replyTo': {'email': email, 'name': naam},
        'subject': f'Nieuwe BouwFlow aanvraag - {bedrijf}',
        'textContent': text,
    }

    try:
        response = requests.post(
            'https://api.brevo.com/v3/smtp/email',
            headers={'api-key': BREVO_API_KEY, 'content-type': 'application/json'},
            json=payload,
            timeout=20,
        )
    except requests.RequestException:
        return jsonify(ok=False, message='Versturen lukt nu niet. Mail anders naar fokkerrik@gmail.com.'), 502

    if response.status_code >= 300:
        return jsonify(ok=False, message='Versturen lukt nu niet. Mail anders naar fokkerrik@gmail.com.'), 502

    return jsonify(ok=True, message='Bedankt! Je aanvraag is verstuurd.')

@app.get('/health')
def health():
    return jsonify(ok=True)

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=int(os.getenv('PORT', '80')))
